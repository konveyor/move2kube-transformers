...
ibmcloud ce application create --name {{ .ServiceName }} --visibility project --registry-secret {{ .RegistrySecret }} --image {{ .RegistryURL }}/{{ .Namespace }}/{{ .ServiceName }}{{ if .Memory }}{{ .Memory }}{{ end }}{{ if .EphemeralStorage }}{{ .EphemeralStorage }}{{ end }}{{ if .NumInstances }} --min-scale {{ .NumInstances }}{{ end }}{{ if .VcapAsEnvSecretName }} --env-sec {{ .VcapAsEnvSecretName }} {{ end }}
